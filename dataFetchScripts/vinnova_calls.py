...
def display_calls(api_key, oppningsdatum_from, oppningsdatum_to):
    ...
    try:
        ...
        if len(df) > 0:
            ...
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            json_filename = os.path.join(OUTPUT_DIR, f"vinnova_calls_{timestamp}.json")
...
def extract_specific_fields(api_key, oppningsdatum_from, oppningsdatum_to):
    ...
    try:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        json_filename = os.path.join(OUTPUT_DIR, f"vinnova_calls_selected_fields_{timestamp}.json")
