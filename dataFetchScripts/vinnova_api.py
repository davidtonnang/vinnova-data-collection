...
OUTPUT_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'dataOutput')
...
class VinnovaAPI:
    ...
    def get_applications(self, beslut_from_date, beslut_to_date):
        try:
            # Construct the URL with only decision date parameters
            url = (f"{self.base_url}/ansokningar"
            ...
            print(f"Error fetching applications from Vinnova API: {str(e)}")
            return None
...
def display_metadata(api_key):
    ...
    try:
        ...
        json_filename = os.path.join(OUTPUT_DIR, f"vinnova_metadata_{timestamp}.json")
...
def display_applications(api_key, beslut_from_date, beslut_to_date):
    # Initialize the API client
    vinnova = VinnovaAPI(api_key)
    
    # Fetch applications
    print(f"Fetching applications with decision dates from {beslut_from_date} to {beslut_to_date}...")
    data = vinnova.get_applications(beslut_from_date, beslut_to_date)
    ...
    try:
        ...
        if len(df) > 0:
            ...
            print("\nVinnova Applications Summary:")
            print("=" * 100)
            pd.set_option('display.max_colwidth', 50)  # Limit column width for better display
            print(display_df.to_string(index=False))
            print("\nTotal number of applications:", len(df))
            
            # Save complete data to JSON file with formatted text
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            json_filename = os.path.join(OUTPUT_DIR, f"vinnova_applications_{timestamp}.json")
            ...
            print("No applications found for the specified date range.")
...
def extract_specific_fields(api_key, beslut_from_date, beslut_to_date):
    ...
    try:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        json_filename = os.path.join(OUTPUT_DIR, f"vinnova_selected_fields_{timestamp}.json")
