...
def display_financed_activities(api_key, beslut_from_date, beslut_to_date):
    ...
    try:
        ...
        if len(df) > 0:
            ...
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            json_filename = os.path.join(OUTPUT_DIR, f"vinnova_financed_activities_{timestamp}.json")
...
def extract_specific_fields(api_key, beslut_from_date, beslut_to_date):
    ...
    try:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        json_filename = os.path.join(OUTPUT_DIR, f"vinnova_financed_activities_selected_fields_{timestamp}.json")
